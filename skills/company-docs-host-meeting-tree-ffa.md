...
> 2. Edit `skills/company-docs-host-meeting-tree-ffa/ranked_choice.py`: populate CANDIDATES with the meeting's numbered items and VOTES with each agent's parsed rankings (verbatim from Part 2).
> 3. Run `python3 skills/company-docs-host-meeting-tree-ffa/ranked_choice.py` to calculate the winner.
...
**Manager:** In the "Your thread to review" section below, expand into one line per attendee using the thread `ts` values recorded in Step 1.
